...
export default function CartSummary() {
	...
	const totalPrice = items.reduce(
		(total, item) =>
			total + Number(item.product.price) * item.cart_item.quantity,
		0,
	);
	const totalDiscountAmount = totalPrice * (discount / 100);
	const totalAmount = totalPrice - totalDiscountAmount;
	...
	return (
		<section
			...
			className="mt-0 rounded-lg bg-gray-50 px-4 py-6 sm:p-6 lg:col-span-5 lg:mt-0 lg:p-8"
		>
			...
			<dl className="mt-6 space-y-4">
				...
				<div className="flex items-center justify-between">
					...
					<dd className="text-sm font-medium text-green-600/80">
						- ₹{totalDiscountAmount.toFixed(2)}
					</dd>
				</div>
				<div className="flex items-center justify-between">
					<dt className="flex gap-1 text-sm text-gray-600">
						Delivery
						<svg
				...
				{items.length > 0 && (
					<div className="flex items-center justify-between">
						...
						<dd className="text-sm font-semibold text-gray-900">{discount}%</dd>
					</div>
				)}
			...
			<div className="mt-3">
				<p className="text-[1rem] font-semibold text-emerald-600">
					You will Save ₹{Math.round(totalDiscountAmount).toFixed(2)} on this
					order
				</p>
			</div>
		</section>
